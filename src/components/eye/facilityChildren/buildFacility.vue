...
<script>
    ...
    export default {
        ...
        methods: {
            treeShow(){
                console.log(this.getfacilityBuild)
                if(this.getfacilityBuild){
                    this.lightCheckout=this.getfacilityBuild
                }
            },
            showBroadCard() {
                console.log(777)
            ...
            async getAllLight(){
                await api.build.getAllBuild().then(res => {
                    ...
                    this.lightList.forEach(item => {
                        item.label = item.building.name
                        item.id =  item.building.id
                        item.type = 'build'
                        item.icon = '../../../static/img/build_big.svg'
                        if (item.regionId) {
