...
<script>
    ...
    export default {
        ...
        methods: {
            treeShow(){
                console.log(this.getfacilityPlant)
                if(this.getfacilityPlant){
                    this.lightCheckout=this.getfacilityPlant
                }
            },
            showBroadCard() {
                console.log(777)
            ...
            async getAllLight(){
                await api.plant.getAllPlant().then(res => {
                    ...
                    this.lightList.forEach(item => {
                        item.label = item.plant.name
                        item.id =  item.plant.id
                        item.type = 'plant'
                        item.icon = '../../../static/img/plant_big.svg'
                        if (item.regionId) {
