...
<script>
    ...
    export default {
        ...
        methods: {
            treeShow(){
                console.log(this.getfacilityTrash)
                if(this.getfacilityTrash){
                    this.lightCheckout=this.getfacilityTrash
                }
            },
            showBroadCard() {
                console.log(777)
            ...
            async getAllLight(){
                await api.dustbin.getAllDustbin().then(res => {
                    ...
                    this.lightList.forEach(item => {
                        item.label = item.dustbinBean.name
                        item.id =  item.dustbinBean.id
                        item.type = 'trash'
                        item.icon = '../../../static/img/trash_big.svg'
                        if (item.regionId) {
