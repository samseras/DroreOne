...
<script>
    ...
    export default {
        ...
        methods: {
            treeShow(){
                console.log(this.getfacilityPark)
                if(this.getfacilityPark){
                    this.lightCheckout=this.getfacilityPark
                }
            },
            showBroadCard() {
                console.log(777)
            ...
            async getAllLight(){
                await api.park.getAllPark().then(res => {
                    ...
                    this.lightList.forEach(item => {
                        item.label = item.parkingBean.name
                        item.id =  item.parkingBean.id
                        item.type = 'park'
                        item.icon = '../../../static/img/park_big.svg'
                        if (item.regionId) {
