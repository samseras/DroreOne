                ...
                <ScrollContainer>
                    <el-table
                        ...
                        <el-table-column label="操作" width="200">
                            <template slot-scope="scope">
                                <span @click="editInfo(scope.row,false,'编辑偏离轨迹告警规则')" class="edit">处理</span> |
                                <span @click="showDetail(scope.row,true,'查看偏离轨迹告警规则')">查看</span> |
        ...
        methods: {
            ...
            searchAnything (info) {
                ...
                if (info.trim() !== '') {
                    ...
                    let obj = {totalNum: this.offtrackList.length}
            ...
            batchEnabled(flag){
                ...
                let choseId = []
                choseId = this.offtrackList.filter(item => {
            ...
            startEndPlan(choseId, flag) {
                ...
                api.alarm.updateAlarmRule(choseId).then(res => {
                    ...
                    choseId.forEach(item => {
                        this.offtrackList.forEach(item1 => {
                            if (item.id === item1.id) {
                            ...
                            for (let i = 0; i < this.choseInfoId.length; i++) {
                                this.offtrackList = this.offtrackList.filter((item, index) => {
                                    if (item.id === this.choseInfoId[i]){
                                        this.offtrackList[index].checked = false
                                        this.offtrackList[index].status = false
                                    }
                ...
                await api.alarm.getAlarmRulesByParameters(this.alarmTypeId).then(res => {
                    ...
                    this.allOfftrackList.forEach(item => {
                        ...
                        if(item.relatedSchedules.length > 0){
                            ...
                            item.relatedScheduleNames =  item.relatedScheduleNames.join(",")
                        }else{
                            item.relatedScheduleNames = ''
                            item.relatedScheduleIds = []
                        }
